...
| PalletChip | `PalletChip` with popover math (delivered − returned − charged ± adj.) | `palletBalance`; the popover is READ-ONLY for every role (equation + «Paddon harakatlari» link into the tab). The pre-bound return action is a first-class header button instead — see §2.4 «Paddon qaytarib olish». A button *inside* the popover was tried and rejected: `PalletChip`'s Popover is uncontrolled (`trigger="click"`), so it would keep floating above the drawer it opened. |
| Paddonlar tab | `PalletStatsPanel` + `DataTable` | `palletStats` (all-time «berilgan − qaytargan [− yo'qotilgan ± tuzatish] = qoldiq» rail) over `GET /pallets/transactions?clientId=:id&page&pageSize`. **NO date window** — a pallet debt is unreturned pieces, not a period. Visible to A·B·G (`pallets.view`): an agent who cannot see his client's pallet debt cannot collect it either. The panel's `extra` slot carries the §2.4 pallet actions on desktop; on a phone they drop to their own full-width row under the panel. Footer link: «Barcha harakatlar →» `/pallets?clientId=:id`. |
| ↳ «Qaysi buyurtmalardan» | `PalletOriginsPanel` | Owner request 2026-08-13: the tab answered «hozir mijozda 15 dona» but never «qaysi 15». `GET /pallets/clients/:id/origins` returns one row per still-open DELIVERY lot — buyurtma № (link) · sana + yosh · zavod · berilgan / qaytargan / yo'qotilgan · **QARZ** — plus an «Excel import» chip when the lot came from a batch (its tooltip names the file + date) and a red «Bekor qilingan» chip when a cancelled order still carries pallets (which should be 0 — the storno continuation clears it). Returns are attributed by the SERVER: the order named on the return row first, otherwise FIFO oldest-lot-first. `unassigned` is a RESIDUAL row («Manba ko'rsatilmagan») so Σ(qarz) + manbasiz ALWAYS equals the chip — the panel can never disagree with the card. Rendered only when there is something to say (open lots or a residual). Phone: one card per lot, QARZ on the right. Roles A·B·G (`pallets.view`). |
...
| ↳ «Kassadan tashqari» solishtirma | `ClientPaidTotalsStrip` + a note over `TransactionsJournal` | Owner complaint 2026-08-13: the strip said «Mijozdan jami olingan 138 621 500 · 8 ta hujjat» while the «Tranzaksiyalar» journal beside it showed 4 rows / 126 121 500, and NOTHING explained the gap (the old caveat named TRANSPORT_DIRECT, of which this DB has zero rows). Cause: the strip counts DOCUMENTS, the journal reads CASH rows, and an imported «шопр учун барди» payment is a real CLIENT_IN with **no cashbox** — it settles the client's debt but never touched a till (85 rows / 253 184 000 across 22 clients). `paymentTotals` now returns `viaKassa` / `offKassa` / `offKassaCount` (+ `offKassaRefunded`), the strip prints «shundan kassaga tushgan X · kassadan tashqari Y (N ta hujjat)» in warning ink, the journal carries a one-line note naming what it cannot show, and the document table gained a «Kassa» column («kassadan tashqari» when null) — mirroring `FactoryDetail`'s bonus carve-out. Invariant: `viaKassa` equals the journal's own KIRIM for that client. |
...
| Paddon qaytarib olish | button (header) + primary button in the Paddonlar tab | `ClientPalletDrawer` mode `return` → `POST /pallets/client-return`. Party LOCKED (no client select — the page *is* the client); qty capped at `palletBalance` with «Mijozda mavjud: N dona», date defaults today, optional note. Info alert: «Pul harakati yo'q — faqat paddon soni hisoblanadi». **Rendered only when `palletBalance > 0`** — at zero there is nothing to take back and the server refuses, so the button is absent rather than disabled (a disabled button reads as «no permission»). Roles A·B·**G** (`pallets.clientReturn`; owner rule 2026-07-30 — the agent collects the pallets, so he records the return). |
| Yo'qotilganini undirish | danger button in the Paddonlar tab (NOT in the header) | `ClientPalletDrawer` mode `lost` → `POST /pallets/charge-lost`. Same cap + a «Dona narxi» `MoneyInput` prefilled from the `palletPriceDefault` setting (fallback: the owner-locked 130 000 — the very number the server would apply), plus the live warning «Mijoz qarziga X qo'shiladi». A·B only (`pallets.mutate`) — it writes MONEY onto the client. Deliberately not in the header: converting pallets to debt is a decision made while LOOKING at the equation, and the header already carries six actions. |
| Qaytarishni bekor qilish | danger button on each `RETURNED_BY_CLIENT` row of the Paddonlar defteri (and on the same rows of `/paddonlar`) | `ReasonModal` → `POST /pallets/transactions/:id/reverse`. The owner's case (2026-08-01): «paddonni bir mijozdan oldik, keyin qarasak boshqa mijoz ekan». The row is NOT deleted — a REVERSAL row is written (`qty` = **+qty**, a signed balance delta) and both rows stay in the ledger, ghosted. Reason REQUIRED (it lands in the storno's `note`). Impact facts: «N dona mijozga qaytadi» · «diller zaxirasi N ga kamayadi» · «pul harakati yo'q» · «qator o'chirilmaydi». Eligibility comes from the SERVER (`fullyReversed` / `remainingQty` on the transactions payload — a scalar, because a row may now carry several partial stornos and an empty array would read as «reversed»), never from the current page's rows. Refused when the pallets have already gone on to a factory (the loose pool would go negative) — the message says to record the RIGHT client's return first. A cancelled order's clamped storno is CONTINUED in the same transaction, so a cancelled order's pallets can never come back to life on the card. Roles A·B·**G** (`pallets.reverseReturn` — same list as the return itself: whoever writes it, fixes it; `assertOwnAgent` scopes the agent to his own client). |
...
`/clients/:id?tab=hisob|buyurtmalar|tolovlar|paddonlar|narxlar&from&to&panel=tolov&page`
(the pallet drawers are LOCAL state, like the edit and price drawers — only `panel=tolov` is URL-borne)
