...
| `POST /pallets/factory-return` `{factoryId,qty,date,unitPrice?,note?}` / `POST /pallets/charge-lost` `{clientId,qty,date,unitPrice?,note?}` | `unitPrice` defaults to 130 000 server-side | Prefill from `GET /settings → palletPriceDefault` (A/B readable) with a deviation hint — the hardcoded frontend constant dies. |
...
| Tab: Paddonlar | `DataTable` + `PalletChip` header | Detail `palletTransactions[]` labeled «oxirgi 50». Columns: Sana · Turi (dot StatusChip from shared pallet map) · Soni (dona) · Narx (dona) · Jami (qty × narx for money rows, — otherwise) · Izoh. Money rows (RETURNED_TO_FACTORY) show their ledger consequence inline: «hisobga +2 600 000». REVERSAL rows ghost-styled, chained. Header: balance chip + one action «Paddon qaytarish». Footer: «To'liq harakatlar →» `/pallets?factoryId=:id`. |
...
| **Paddon qaytarish** | pallet factory-return modal (§4.5) pre-bound to this factory | `POST /pallets/factory-return`. Shows current → post-action accountability and the credit preview «Zavod hisobiga: +2 600 000 so'm (taxminiy — server tasdiqlaydi)». |
...
accountable for (factories), and every movement — with the two money-bearing flows
(Undirish, Zavodga qaytarish) previewing their exact ledger consequence. In-kind counts are
never mixed with money. Roles: A B full · G read-only own scope.
...
│ Sana   Turi              Mijoz      Zavod    Soni  Narx(dona)  Jami  Hujjat│
│ 05.07  ● Mijozga yuborildi Жамол Ург.  —       19      —        —  ORD-000141│
│ 06.07  ● Zavodga qaytarildi  —      CAOLS KS   20   130 000  2 600 000  — │
│ ── Sahifa jami: kirim 39 · chiqim 20 · pul: 2 600 000 ──                   │
└────────────────────────────────────────────────────────────────────────────┘
```
...
| Totals | `TotalsRow` | «Sahifa jami» (or «oxirgi 200 yozuv» when the client window is active): net in-kind delta (kirim/chiqim dona) + Σ money of money-bearing rows. |
...
| Zavodga qaytarish | modal | `POST /pallets/factory-return`. Zavod (pre-bound) · Soni · **Dona narxi** `MoneyInput` prefilled from `GET /settings → palletPriceDefault` (fallback 130 000) with deviation hint «standart: 130 000» when edited · Sana · Izoh. Preview: current → post-action accountability + **«Zavod hisobiga kredit: 2 600 000 so'm (taxminiy — server tasdiqlaydi)»** — the one ledger posting named. |
| Undirish | modal, danger confirm | `POST /pallets/charge-lost`. Mijoz (pre-bound) · Soni · Dona narxi (same prefill/hint) · Sana · Izoh. Warning block: «Mijozga PUL qarzi yoziladi: +1 560 000 so'm» + post-action pallet balance. Confirm «Undirish» danger, never default-focused. |
...
adjacent to, never inside, money; money enters only via the two explicit flows, each modal
naming its single ledger posting; order cancellation reverses only the order's own two
...
1200↓: boards stack vertically; movements low-priority columns (Narx, Jami, Izoh) fold into
row expand.
...
| Hardcoded `DEFAULT_PALLET_PRICE = 130000` in the frontend | **Dies** → prefill from `palletPriceDefault` setting + deviation hint | Settings edit silently didn't change the prefill (pain point); single source of truth. |
...
| No Turi/date filters, no totals, no line totals | → labeled-window Turi/Sana filters, `TotalsRow`, Jami column with the ledger figure | «Did the debt really move by 390 000?» becomes readable in place. |
