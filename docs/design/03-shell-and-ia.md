...
| `/pallets` | A B G(read, scoped) | In-kind ledger: balances (clients \| factories) side by side, one primary action per side + row kebabs; movements table with date-range + type filters and totals footer (net in-kind delta; qty × narx line totals for CHARGED_LOST rows — the only money-bearing type). Mutation modals show current → post-action balance (warn, don't block, on negative); **«Zavodga qaytarish» has no price field at all** — a factory return moves ZERO money, it only discharges a COUNT (the API rejects a `unitPrice` with 400). Only the lost-pallet charge («Yo'qotilganini undirish», CHARGED_LOST → client debt) carries a unit price, prefilled from the `palletPriceDefault` setting (fallback 130 000) with a deviation hint. |
...
| `/settings` | A (write), B (read-only) | Per-field save affordance (each key PUTs independently with inline ✓/✗ — partial-write confusion dies); `saleMarginMinPct` carries an amber «hozircha tizimda qo'llanilmaydi» badge until the backend consumes it; effective values cross-referenced («qayerda ishlatiladi» hints; the lost-pallet charge modal reads `palletPriceDefault` — it is the price billed to a client for a LOST paddon, not an order price and not a factory-return price). |
...
| Pallets.tsx | Stays; row-launched actions with current→post-action balance; date+type filters; totals footer; setting-driven unit price **on the lost-pallet charge only** — the factory-return modal loses its price field (money-free, count-only). |
