...
- **Order (imported)** — One Товар row = one truckload, created directly as a completed sale with historical date; posts CLIENT +saleTotal and FACTORY −costTotal ledger entries.
  - Fields: orderNo (ORD-xxxxxx from order_no_seq); status=COMPLETED; date/completedAt = workbook date; saleTotal; costTotal (m³×cost — blocks only, palletPrice booked 0); costStatus=PROVISIONAL; transportMode=DEALER_ABSORBED; transportCost; transportPaidStatus; items[0].pricePending (true when shipped before price agreed); importBatchId
...
- Each Товар row becomes a COMPLETED order (historical date, ORD-###### from order_no_seq) posting CLIENT +saleTotal and FACTORY −costTotal; costTotal = round2(m³×costPrice) — blocks only, palletPrice is booked 0; costStatus=PROVISIONAL, provisionalPriceKind=FACTORY_BANK; transportMode always DEALER_ABSORBED.
...
- **AGENT (field sales agent, bound to one Agent record via agentId)**: Sales workspace scoped to own agent only — every list query is filtered by user.agentId server-side. PAGES: / (dashboard), /orders(+new/:id), /clients(+:id), /agents/:id (own agent detail), /payments, /debts, /pallets, /profile. Blocked pages: /agents list, catalog, /bonus, /expenses, /kassa, /reports, /import, /users, /settings (403 screen). ACTIONS: GET/POST orders, order comments — but NOT order edit (PUT), NOT price patch, NOT delete, and NO status stepping (the status workflow was removed 2026-07-22); clients GET/POST/PUT (created clients force-bound to own agentId), no delete, no aliases/prices; payments GET/POST (agentId forced to own); debts /clients + /statement (not /summary); pallets balances + transactions (read) + **POST /pallets/client-return for his OWN clients** (owner rule 2026-07-30 — the agent physically collects the pallets; `assertOwnAgent` ⇒ 403 on a foreign client), but NOT charge-lost and NOT factory-return (money / company accountability); GET /agents/me (own record) and GET /agents/:id; read-only reference lists: factories, products, vehicles, regions, procurement matrix is FIN-only in v3 (403); dashboard summary + trends (scoped). No settings access at all.
...
- Settings writes are ADMIN-only and strictly whitelisted to 4 keys with per-key validation: agentDebtLimitDefault (>= 0 or null; null = unlimited, 0 = new credit orders blocked; per-agent Agent.debtLimit overrides it; enforced at order creation), truckCapacityPallets (integer 1–40; default 19; used when a vehicle/route has no own capacity), saleMarginMinPct (0–100, 2dp), palletPriceDefault (> 0; the lost-pallet charge falls back to 130,000 so'm if unset or 0 — orders never carry pallet money). Unknown keys are rejected.
...
- **OrderItem** — One product line on the truck; holds both sale side (client price) and cost side (factory price) with full pricing provenance
  - Fields: productId; quantityM3 (3dp; explicit value wins over palletCount × m3PerPallet); palletCount; palletPrice (always 0 on new orders — pallets are in-kind; column kept for historical rows); listPricePerM3 (price-book reference at creation, discount derivable); salePricePerM3 (6dp; back-solved for lump sums); saleTotal (2dp; lump sums stored EXACTLY); pricePending (shipped before price agreed); provisionalPriceKind (FACTORY_CASH/FACTORY_BANK from intended payment method); costPricePerM3 / finalCostPricePerM3; costTotal (m³ × costPrice — blocks only, pallets add nothing)
...
- **Soft-cancel order** (ADMIN, ACCOUNTANT; rare)
  ...
  1. Confirm modal (`CancelOrderModal`): shows the order's money map and asks ONE question — «To'langan pullar avansda qoladimi?»; mandatory reason textarea
  ...
  1. MONEY, owner rule 2026-07-29 (replaces the 2026-07-26 rule, which returned the factory's money in BOTH modes). ONE question decides BOTH sides, because both are the same physical fact — where the money actually sits
  1. REFUND («Ha — avansda qoladi», default): cancelling MOVES NO MONEY. The client's money stays with us and reads as a BALANCE CREDIT (his advance); the money we wired stays AT THE FACTORY as our advance IN ITS OWN CHANNEL (naqd → naqd, o'tkazma → o'tkazma) — the ADVANCE_DRAW pair reversed above already put it back there, so nothing else is written and NO cashbox moves. The factory payment document stays LIVE and its money is drawable onto the next order. The order also RECORDS the chosen mode (`Order.cancelMoneyMode`) so the dead card can still say where the money went; rows cancelled before 2026-07-29 carry NULL and claim nothing. BONUS-funded settlements are never advance (their BONUS_OFFSET row sits on PAYABLE and the wallet is already spent) and are left alone in both modes; the modal subtracts that slice from «zavodda qoladigan avansimiz» rather than promising an advance that will not appear
  1. VOID_ALL («Yo'q — to'lamagandek bo'lsin»): a data-entry cleanup — BOTH payment documents are stornoed. The factory money returns to the EXACT cashbox it left and the EXACT advance channel it sat in (the other channel is never touched); when the paying document belongs wholly to this order it is STORNOed rather than answered with a new refund document, so the box nets out — «huddi kassaga pul kirmagandek, kassadan chiqmagandek». A payment only partly consumed by this order keeps its remainder standing at the factory. The client's balance and the kassa both return to zero — as if the order had never been placed
  1. TRANSPORT_DIRECT (client paid the driver himself) is voided as a document in BOTH modes — no phantom client credit
  1. LIMIT OF VOID_ALL, stated on screen rather than discovered later: only a payment WHOLLY belonging to this order can be stornoed. Client money auto-settles oldest-order-first, so one CLIENT_IN document routinely funds two orders — voiding it would reopen the other one. The server therefore skips it and the money stays as the client's advance; the modal detects the shared document and says so instead of promising «to'lamagandek bo'ladi»
...
- Pallets are owed IN-KIND: clients owe pallet counts (delivered − returned − charged-lost ± signed adjustments), never pallet money; the factory side is a COUNT too (a return there posts nothing), so pallet money exists ONLY via explicit CHARGED_LOST ledger charges
...
Pallets are owed IN KIND (counts, never money) — an explicit owner decision. Every order automatically posts two pallet movements in the order's transaction: RECEIVED_FROM_FACTORY (dealer becomes accountable to the factory) and DELIVERED_TO_CLIENT (client now holds our pallets). Client balance = delivered − returned_by_client − charged_lost ± adjustments/reversals; factory balance = received − returned_to_factory ± adjustments/reversals. Money enters through ONE explicit flow, posting exactly one linked LedgerEntry: CHARGED_LOST (converts lost pallets into client money debt at ~130 000 UZS/pallet). RETURNED_TO_FACTORY moves ZERO money — it only discharges the pallet COUNT owed to that factory. Order cancellation creates compensating REVERSAL rows for the order's own two movements only — physical client returns are never undone.
...
- **PalletTransaction** — In-kind movement ledger for returnable pallets. Balances are pure sums over these rows — no stored counters. unitPrice is set on exactly ONE type, CHARGED_LOST, linked 1:1 to a CLIENT LedgerEntry; every factory-side row is count-only.
  - Fields: type; qty Int (signed for ADJUSTMENT/REVERSAL); date + at; clientId? / factoryId? / orderId?; unitPrice Decimal(18,2)? (CHARGED_LOST **only** — DB CHECK `pallet_factory_return_moneyless` forbids it on RETURNED_TO_FACTORY); reversalOfId (**1:N** since 2026-08-13 — a delivery row may collect several PARTIAL stornos; see the cancel rule below) + reversalOfType (the source row's type, denormalised so a partial-unique index can keep RETURNED_BY_CLIENT / CHARGED_LOST reversible exactly once); note; createdById
  - States: RECEIVED_FROM_FACTORY (auto with order) | DELIVERED_TO_CLIENT (auto with order) | RETURNED_BY_CLIENT (manual, no money) | RETURNED_TO_FACTORY (manual, **no money** — discharges a COUNT) | CHARGED_LOST (manual, posts client debt) | ADJUSTMENT (signed) | REVERSAL (signed, from order cancel)
...
- **OrderItem (cost side — adjacent but load-bearing)** — Where 'stock valuation' actually lives: quantityM3 (wins over palletCount × m3PerPallet when both given) drives costTotal = m³ × costPricePerM3. Pallets are counted, never valued: they are NOT billed to the client (saleTotal excludes them) and NOT charged by the factory (palletPrice is booked 0).
  - Fields: quantityM3 Decimal(12,3); palletCount; palletPrice (always 0 on new orders — retained for historical rows); provisionalPriceKind (default FACTORY_BANK); costPricePerM3 / finalCostPricePerM3; costTotal
...
- **Automatic pallet flow with each order (system-side)** (AGENT, ACCOUNTANT, ADMIN (via order creation); many times/day)
  ...
  1. Pallets add NOTHING to the order's costTotal (palletPrice is booked 0 — costTotal = m³ × costPricePerM3 only); the client is never billed for pallets in saleTotal either
...
- **Return pallets to factory (COUNT ONLY — no money)** (ADMIN, ACCOUNTANT; weekly)
  1. Paddonlar page → 'Zavodga qaytarish' (header or factory-row button); same form on Qarzlar › Paddonlar and on the factory card
  1. Modal: factory, qty (capped at min(loose in-hand, what we owe that factory)), date, note — **no price field**; the impact preview says «Pul harakati yo'q — faqat paddon soni hisoblanadi»
  1. Save → ONE RETURNED_TO_FACTORY row and nothing else: no LedgerEntry, no unitPrice, the factory's money balance is untouched. Sending a `unitPrice` to the endpoint is a 400 (2026-07-23).
...
PALLETS (/pallets): vertical stack. Header Flex row: H3 'Paddonlar' left; right side three global buttons: 'Qaytarish qabul qilish' (A/B/G since 2026-07-30 — `pallets.clientReturn`), 'Zavodga qaytarish', red 'Yo'qotilganini undirish' (both A/B — `pallets.mutate`). Below, a 15/9 two-column Row: left small Card 'Mijozlardagi paddonlar' with client-side search in header and a small Table (client name → link to /clients/:id, balance right-aligned bold — orange if >0, red if <0, client-paginated 15 rows) plus a 300px per-row action column duplicating two of the header buttons with the client prefilled; right small Card 'Zavodlar oldidagi hisobdorlik' (hidden for agents) with a compact unpaginated factory balance Table and per-row 'Zavodga qaytarish' button. Bottom: full-width Card 'Paddon harakatlari' — two filter Selects (client, factory; searchable) and a server-paginated movement Table: Sana, Turi (colored Tags: Zavoddan olindi/blue, Mijozga yuborildi/cyan, Mijoz qaytardi/green, Zavodga qaytarildi/purple, Undirildi/red, Tuzatish, Storno/volcano), Mijoz (link), Zavod, Soni, Narx (dona), Buyurtma (link), Izoh. No date-range or type filter, no totals row, no export. All three mutations are centered Modals with vertical forms; client/factory Selects embed the live balance in the option label ('Name (balans: N)'); charge-lost shows a live computed money preview in a warning Alert (client debt) with the unit price prefilled from the `palletPriceDefault` setting (fallback 130 000); factory-return has no price field and previews «Pul harakati yo'q — faqat paddon soni hisoblanadi».
...
- [FIXED 2026-07-30] Pallet unit price 130 000 was hardcoded in the Pallets page's charge-lost modal (DEFAULT_PALLET_PRICE constant), while the API's charge-lost reads the editable 'Yo'qolgan paddon narxi' (palletPriceDefault) setting — changing the setting silently did NOT change the prefill. Both charge-lost forms (the /pallets modal and the client card's ClientPalletDrawer) now read `GET /settings → palletPriceDefault`, and the single shared fallback constant is `FALLBACK_LOST_PALLET_PRICE` (components/ClientPalletDrawer.tsx), equal to the server's DEFAULT_PALLET_UNIT_PRICE.
  - Suggestion: Prefill the charge-lost modal from the palletPriceDefault setting (single source of truth) and show a hint when the entered price deviates from it. The factory-return modal has no price field at all and must never gain one.
...
- [medium] The one money-bearing pallet action (CHARGED_LOST) doesn't surface its ledger consequence afterwards: the transactions table shows unitPrice but never the line total (qty × price) nor a link to the posted LedgerEntry, so verifying 'did the client debt really move by 390 000?' requires mental math plus a trip to the Debts/Client page.
  - Suggestion: Add a computed total column for CHARGED_LOST rows and link to the ledger entry / client balance impact. (Factory-side rows must stay money-free — see the pallet rule below.)
...
- Order cancellation posts compensating REVERSAL rows ONLY for the order's own RECEIVED_FROM_FACTORY / DELIVERED_TO_CLIENT movements (qty negated, linked via reversalOfId). Client returns and lost charges are standalone physical/financial facts and are never reversed by a cancel. The reversal is CLAMPED to what the client still holds (his balance may never go negative), and the clamped remainder is CONTINUED automatically the next time his count rises — a cancelled return/charge storno, or a new delivery (2026-08-13). That is why one source row may carry several reversals: without the continuation, a cancelled order's pallets stayed on the client card forever with no traceable source. An order EDIT reverses in FULL instead (no clamp) and then re-posts, since a clamped edit double-counted the order's pallets; if the result would drive the client negative the edit is refused.
- Money enters the pallet world through exactly ONE explicit flow (2026-07-21): CHARGED_LOST → CLIENT account debit (client owes qty × unitPrice), a manual ADMIN/ACCOUNTANT action, default 130 000 UZS. RETURNED_TO_FACTORY posts NO ledger money at all — the factory never pays for pallets, so a return discharges a COUNT and nothing else. Enforced end-to-end since 2026-07-23: the DTO REJECTS a unitPrice (400), no UI asks for one, and two DB CHECKs (`pallet_factory_return_moneyless`, `ledger_no_pallet_return_credit`) make the old behaviour unwritable. `LedgerSource.PALLET_RETURN_CREDIT` is retired: historical rows keep rendering, nothing writes it.
- Order item cost = m³ × costPricePerM3 ONLY. `OrderItem.palletPrice` is booked as 0 on every new order (pallets are an in-kind deposit, never a cost component); the column survives for historical rows. Cost is PROVISIONAL at creation (default kind FACTORY_BANK) and is FIXED only at factory-payment allocation (owner explicitly chose this over lock-at-creation); the provisional→final delta posts a COST_ADJUSTMENT ledger entry.
...
- POST /pallets/client-return — RETURNED_BY_CLIENT (qty, date, optional orderId/note; no money). ADMIN/ACCOUNTANT **and AGENT** (owner rule 2026-07-30: the agent physically collects the pallets, so he records the return) — an AGENT is row-scoped to his own clients (`assertOwnAgent` ⇒ 403 on a foreign client), and a supplied `orderId` must belong to that same client (⇒ 400)
- POST /pallets/factory-return — RETURNED_TO_FACTORY, COUNT ONLY (no ledger entry, no money); a `unitPrice` in the body is REJECTED with 400
- POST /pallets/charge-lost — CHARGED_LOST + CLIENT ledger debit (unitPrice default 130 000)
- POST /pallets/transactions/:id/reverse — storno of a CLIENT-side movement; body `{ reason }` (required). Accepts exactly two types and refuses the rest with an actionable 400: **RETURNED_BY_CLIENT** (owner rule 2026-08-01 — "the pallets came from a different client"): writes a REVERSAL row with **+qty** (a signed balance delta), pallets go back onto that client, no money, and it is REFUSED when the dealer's loose stock is already below qty (they were shipped on to a factory). **CHARGED_LOST** (owner rule 2026-08-04 — "the pallets turned up / the wrong client was charged"): same +qty pallet reversal **plus** a reversal of the single linked PALLET_CHARGE LedgerEntry (`LedgerService.reverse(..., { palletTransactionId })` — the column is @unique, so the reversal must point at the NEW pallet row), so the client's money debt drops by exactly the charged amount; no loose-stock limit applies (a lost pallet never came back to the yard). Nothing is deleted — `reversalOfId` is UNIQUE, so a row can be cancelled once. Roles: RETURNED_BY_CLIENT is A/B/**G** (`pallets.reverseReturn` — whoever records it fixes it, `assertOwnAgent` row-scopes the AGENT); CHARGED_LOST is A/B only (`pallets.reverseCharge`, 403 for an AGENT even on his own client — it moves money). Response: `{ reversedKind, clientPalletBalance, reversedAmount }`. A clamped cancelled-order reversal is CONTINUED afterwards, so a cancelled order's pallets can never resurrect on the client card.
...
- **Svod report (computed)** — Master reconciliation report = workbook's Свод Завод. Factory block + per-agent client blocks + grand totals + two identity checks that must be 0.
  - Fields: factories[]: goods (m³ × finalCostPricePerM3??costPricePerM3), pallets money (always 0 — pallets are in-kind), goodsWithPallets (therefore ≡ goods), paidToFactory (FACTORY_OUT incl. BONUS offsets), factoryBalance (current ledger); agents[]: rows{client, goods, payments, driverDirect, balance, palletBalance} + subtotal; totals; checks.goodsIdentity / checks.paymentsIdentity (must be 0); from/to bound flows only; balances always current
- **OrdersRegister row (computed)** — Flat one-row-per-order/truck ledger (Товар sheet shape) for review and xlsx export.
  - Fields: orderNo, date, status, agent, client, factory, plate, driver, sizes; m3, costPrice & salePrice (back-solved per-m³, 6dp); costTotal + costStatus (provisional until factory-payment allocation); pallets (count), palletMoney (always 0 — pallets are in-kind); saleTotal, transportCost, transportCharge, transportPaidStatus; goodsProfit = saleTotal − costTotal
...
- **Run the Svod reconciliation report** (ADMIN, ACCOUNTANT; daily)
  ...
  1. Review 'Zavodlar bloki' table: per-factory goods at cost / pallet money (always 0 — pallets are in-kind) / total / paid / current balance, with a Jami summary row
...
- **Review / export the orders register** (ADMIN, ACCOUNTANT; daily)
  ...
  1. Scan the 22-column horizontally-scrolling table (m³, cost & sale per-m³ and totals, cost status, pallets (count; pallet money is always 0 — in-kind), transport cost/charge/status, per-order goods profit, order status)
...
- Goods profit = saleTotal − costTotal (costTotal is blocks only — pallets carry no money; the dashboard definition); net profit subtracts transportCost, which is priced INSIDE saleTotal in every live mode. Reports must label which profit definition is used.
...
- **LedgerEntry (account=FACTORY)** — Immutable settlement postings with the factory. Balance is a sum, never stored; corrections are compensating reversal entries carrying the original business date.
  - Fields: date + at; source (ORDER_COST, COST_ADJUSTMENT, PAYMENT, PAYMENT_VOID, BONUS_OFFSET, ADVANCE_DRAW, ORDER_CANCEL, ADJUSTMENT, OFFBOOK_ADJUSTMENT, IMPORT — plus retired PALLET_RETURN_CREDIT on historical rows only); amount (signed: >0 dealer's asset/advance, <0 dealer owes); factoryId; orderId / paymentId / palletTransactionId links; reversalOfId
...
- **OrderItem (cost side)** — Carries the purchase-pricing lifecycle: provisional cost at creation (intended-method price from the versioned ProductPrice book at order date), finalized by allocation; the PERCENT bonus base is Σ quantityM3 × (finalCostPricePerM3 ?? costPricePerM3), blocks only.
  - Fields: quantityM3 (12,3); palletCount / palletPrice (always 0); provisionalPriceKind; costPricePerM3; finalCostPricePerM3; costTotal = m³×costPrice (pallets add nothing)
  - States: order.costStatus: PROVISIONAL | PARTIAL | FINAL
- **PalletTransaction (factory side)** — In-kind pallet accountability with the factory: pallets received with purchases vs physically returned. PURELY A COUNT — a return posts no ledger entry and moves no money; the factory pays nothing for pallets.
  - Fields: type (RECEIVED_FROM_FACTORY / RETURNED_TO_FACTORY / ADJUSTMENT / REVERSAL); qty; factoryId; date — **no unitPrice** (DB CHECK forbids it on the factory side)
...
- **Return pallets to factory (count only, no settlement)** (ADMIN, ACCOUNTANT; weekly)
  1. From the Pallets page, Qarzlar › Paddonlar, or the factory card, record RETURNED_TO_FACTORY with qty and date — there is no price to enter
  1. Posts ONE PalletTransaction. No LedgerEntry, no advance, no balance change: only the pallet COUNT owed to that factory drops
...
- **LedgerEntry** — Immutable single source of truth for all party balances. Signed postings: >0 = asset for dealer (client owes us / our advance at factory), <0 = dealer's liability. Balance(party) = Σ amount. Corrections only via reverse() rows carrying the ORIGINAL business date.
  - Fields: account (CLIENT|FACTORY|VEHICLE); source (ORDER_SALE, ORDER_COST, COST_ADJUSTMENT, TRANSPORT_CLIENT_DIRECT, TRANSPORT_COST, legacy TRANSPORT_CHARGE, PAYMENT, PAYMENT_VOID, ORDER_CANCEL, PALLET_CHARGE, BONUS_OFFSET, ADVANCE_DRAW, ADJUSTMENT, OFFBOOK_ADJUSTMENT, IMPORT — retired PALLET_RETURN_CREDIT is blocked by CHECK, historical rows only); amount (signed Decimal 18,2, never 0); date (business) vs at (recorded); clientId/factoryId/vehicleId (exactly one, matching account — SQL CHECK); orderId, paymentId, palletTransactionId; reversalOfId (unique)
  - States: posted | reversed (has reversedBy)
- **PalletTransaction** — In-kind pallet ledger (counts, not money). Client balance = Σ DELIVERED_TO_CLIENT − RETURNED_BY_CLIENT − CHARGED_LOST + signed ADJUSTMENT/REVERSAL; factory balance = RECEIVED_FROM_FACTORY − RETURNED_TO_FACTORY ± signed rows. CHARGED_LOST is the ONLY type that posts a linked money LedgerEntry; the factory side is count-only.
  - Fields: type, qty (positive for directional types, signed for ADJUSTMENT/REVERSAL); clientId/factoryId/orderId; unitPrice (default 130,000 UZS — charge-lost ONLY, forbidden on factory returns by DB CHECK); reversalOfId
...
- **AppSetting** — Runtime knobs: agentDebtLimitDefault (null=unlimited), truckCapacityPallets (19), saleMarginMinPct, palletPriceDefault (130,000 — the LOST-pallet charge to a client, not an order or factory-return price).
...
- **Pallet management** (ADMIN, ACCOUNTANT (mutations), AGENT (view own); weekly)
  ...
  1. Dealer returns pallets to factory → 'factory-return' (count + date ONLY) — no money, no ledger entry: it just discharges the pallet COUNT owed to that factory
...
- [medium] The three-sided + pallet 'Свод' view the owner lives in doesn't exist as one screen: Debts covers client rows well, but factory advance/debt, vehicle debts, bonus wallets and factory pallet accountability are scattered across Factories detail, Bonus and Pallets pages — reproducing the workbook's one-glance summary takes 4+ page visits.
  - Suggestion: One consolidated balances dashboard: clients (money + pallets + overdue), factories (money + the pallet COUNT we owe them — never pallet money + bonus wallet), vehicles — each row linking to its statement.
...
- PALLETS IN-KIND — Rule: pallets are an in-kind deposit on BOTH sides — clients owe a pallet COUNT and the dealer owes each factory a pallet COUNT, never pallet money: saleTotal never includes pallets; client pallet balance = Σ delivered − Σ returned − Σ charged-lost ± signed adjustments. Money appears via exactly ONE explicit flow posting one linked LedgerEntry: CHARGED_LOST (client is charged qty × unitPrice, default 130,000 UZS from the palletPriceDefault setting — converts in-kind debt to money debt). RETURNED_TO_FACTORY moves ZERO money — no LedgerEntry, no unitPrice, no growth of the dealer's advance; it only discharges the COUNT (DB CHECKs pallet_factory_return_moneyless + ledger_no_pallet_return_credit enforce it and the API rejects a unitPrice with 400). The dealer's own pallet cost is NOT money either: order costTotal = blocks only (palletPrice is booked 0 on every new order), so the factory balance carries no pallet component. Order cancel reverses only that order's delivery movements — physical returns and charges survive. Why: owner rule 2026-07-21/23, superseding the earlier workbook reading (Поддон subsystem, Завод Остаток 'с паддон') recorded 2026-07-11. UI: pallet counters must appear next to money balances on client rows/statements; charge-lost is a deliberate privileged action with a visible unit price; the factory-return form has NO price field and must state that no money moves.
...
- POST /api/pallets/client-return — pallet return (ADMIN/ACCOUNTANT/AGENT — AGENT scoped to his own clients); POST /api/pallets/factory-return | charge-lost — company accountability / client money debt (ADMIN/ACCOUNTANT)
...
- No block/product return entity exists anywhere. The only "return" machinery in schema.prisma is for PALLETS: PalletEventType RETURNED_BY_CLIENT / RETURNED_TO_FACTORY (schema.prisma:138-139) plus the RETIRED LedgerSource PALLET_RETURN_CREDIT (historical rows only — a factory return now posts no ledger entry at all) — pallets are in-kind count balances per the owner's rule.
...
1. Single ledger write path, zero manual endpoints. LedgerService.post (C:/Users/mello/Documents/GitHub/smartblok/apps/api/src/common/ledger.service.ts:33) is the only LedgerEntry writer ("The single write-path for balance-affecting postings... Balances are sums, never stored"). Its complete caller set: orders.service.ts (ORDER_SALE, TRANSPORT_CHARGE, ORDER_COST, TRANSPORT_COST), payments.service.ts (PAYMENT kinds + COST_ADJUSTMENT), pallets.service.ts (PALLET_CHARGE only — a factory return posts nothing), bonus.service.ts (BONUS_OFFSET), import.service.ts (same sources tagged with importBatchId). No controller (debts.controller.ts has only GET summary/clients/statement) exposes a posting endpoint. Since clientBalance/factoryBalance/vehicleBalance are pure SUMs over LedgerEntry, a party with no history has an unfixable zero balance.
