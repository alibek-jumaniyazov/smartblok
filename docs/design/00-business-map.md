...
- **Cancel order (soft-cancel)** (ADMIN, ACCOUNTANT; rare)
  ...
  1. Active payment allocations voided — the money stays on the client's account as unallocated credit; cash stays in the kassa (no auto-refund; explicit CLIENT_REFUND exists if needed)
