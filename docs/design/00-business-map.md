...
- [medium] Pallet unit price 130 000 is hardcoded in the Pallets page's charge-lost modal (DEFAULT_PALLET_PRICE constant), while the API's charge-lost reads the editable 'Yo'qolgan paddon narxi' (palletPriceDefault) setting with the same 130 000 as code fallback — changing the setting silently does NOT change the charge-lost prefill.
