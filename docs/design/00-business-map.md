...
- **PalletTransaction** — In-kind movement ledger for returnable pallets. Balances are pure sums over these rows — no stored counters. unitPrice is set on exactly ONE type, CHARGED_LOST, linked 1:1 to a CLIENT LedgerEntry; every factory-side row is count-only.
  - Fields: type; qty Int (signed for ADJUSTMENT/REVERSAL); date + at; clientId? / factoryId? / orderId?; unitPrice Decimal(18,2)? (CHARGED_LOST **only** — DB CHECK `pallet_factory_return_moneyless` forbids it on RETURNED_TO_FACTORY); reversalOfId (1:1, cancel trail); note; createdById
