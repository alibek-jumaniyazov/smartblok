...
export default function Reports() {
  ...
  const { data: facPay } = useQuery({ queryKey: ['facpay'], queryFn: () => endpoints.payments({ type: 'FACTORY' }) });

  return (
    <div>
      <PageHeader title="Hisobot — Svod" subtitle="Agentlar yakuni va zavod bilan solishtiruv" breadcrumb={['Hisobotlar', 'Svod']} />
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard label="Zavod qoldig'i" value={Math.max(0, svod?.totals?.factoryBalance ?? 0)} suffix="so'm" tone="red" icon={<Factory size={20} />} />
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-line text-left text-[11px] uppercase tracking-wide text-muted">
                  <tr><th className="py-2.5">Agent</th><th>Guruh</th><th className="text-right">Sotuv</th><th className="text-right">To'langan</th><th className="text-right">Qoldiq</th></tr>
                </thead>
        ...
        <Card>
          ...
          {!facPay ? <TableSkeleton /> : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-line text-left text-[11px] uppercase tracking-wide text-muted">
                  <tr><th className="py-2.5">Sana</th><th>Zavod</th><th>Izoh</th><th className="text-right">Summa</th></tr>
                </thead>
                <tbody className="divide-y divide-line-soft">
                  {facPay.map((p: any) => (
                    <tr key={p.id} className="hover:bg-hover">
                      ...
                      <td className="text-muted">{p.payerName ?? '—'}</td>
                      <td className="text-right font-semibold tabular-nums">{fmtUZS(p.amount)}</td>
                    </tr>
                  ))}
                  {facPay.length === 0 && <tr><td colSpan={4} className="py-10 text-center text-faint">Yozuv yo'q</td></tr>}
                </tbody>
              </table>
            </div>
          )}
