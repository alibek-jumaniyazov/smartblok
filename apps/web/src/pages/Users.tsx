...
const empty = { email: '', name: '', role: 'AGENT', phone: '', password: '', agentId: '', active: true };
...
export default function Users() {
  ...
  function openEdit(u: any) { setEditing(u); setForm({ ...empty, ...u, password: '', agentId: u.agentId ?? '' }); setOpen(true); }
  function close() { setOpen(false); setEditing(null); setForm(empty); }

  const columns: Column<any>[] = [
    { key: 'name', header: 'Ism', render: (u) => (
      <div className="flex items-center gap-3">
        ...
        <div><p className="font-medium text-content">{u.name}</p><p className="text-xs text-faint">{u.email}</p></div>
      </div>
    ) },
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumb={['Tizim', 'Foydalanuvchilar']}
