...
export default function VehicleDetail() {
  ...
  const [form, setForm] = useState<any>({ method: 'CASH', amount: 0, date: new Date().toISOString().slice(0, 10) });
  const { data: vehicle } = useQuery({ queryKey: ['vehicle', id], queryFn: () => endpoints.vehicle(id as string) });
  const { data: orders } = useQuery({ queryKey: ['orders', 'vehicle', id], queryFn: () => endpoints.orders({ vehicleId: id }) });
  const { data: payments } = useQuery({ queryKey: ['payments', 'vehicle', id], queryFn: () => endpoints.payments({ type: 'VEHICLE', vehicleId: id }) });

  const pay = useMutation({ mutationFn: () => endpoints.createPayment({ type: 'VEHICLE', vehicleId: id, ...form }), onSuccess: () => { qc.invalidateQueries(); setOpen(false); toast("Moshinaga to'lov saqlandi"); } });
  const set = (k: string, v: any) => setForm((f: any) => ({ ...f, [k]: v }));

  if (!vehicle) return <TableSkeleton rows={8} />;
  const delivered = (orders ?? []).filter((o: any) => ['DELIVERED', 'COMPLETED'].includes(o.status));
  const owed = delivered.reduce((s: number, o: any) => s + o.transportFee, 0);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KpiCard label="Xizmat haqi" value={owed} suffix="so'm" tone="teal" hint={`${delivered.length} reys`} />
      ...
      <Card className="mt-6">
        <CardTitle>Tashigan buyurtmalar ({orders?.length ?? 0})</CardTitle>
