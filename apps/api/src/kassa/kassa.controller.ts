...
export class KassaController {
  ...
  @Get('transactions') transactions(@Query('cashboxId') cashboxId?: string) {
    return this.service.transactions(cashboxId ? Number(cashboxId) : undefined);
  }
  @Post('transactions') createTransaction(@Body() d: any) { return this.service.createTransaction(d); }
  @Delete('transactions/:id') removeTransaction(@Param('id', ParseIntPipe) id: number) { return this.service.removeTransaction(id); }
}
