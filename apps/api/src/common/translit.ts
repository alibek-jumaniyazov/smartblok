// common/translit.ts — O'zbek lotin → kirill transliteratsiyasi (SERVER nusxasi).
//
// Bu fayl apps/web/src/lib/translit.ts ning AYNAN nusxasi. Sabab: butun loyiha
// matni lotin o'zbekchada yozilgan va kirill jonli transliteratsiya bilan olinadi;
// Excel eksporti serverda yasaladi, shuning uchun o'sha qoidalar shu yerda ham
// kerak. Ikkalasi bir xil turishi shart — biriga tuzatish kiritsangiz, ikkinchisiga
// ham ko'chiring.
//
...
/**
 * Eksport uchun yorliq o'girgich.
 *
 * `toCyrillic` {…} ichidagi matnni tegmasdan ko'chiradi (frontendda interpolatsiya
 * kaliti uchun). Shu xossani BREND/KOD nomlarini himoyalash uchun ishlatamiz:
 * «{SmartBlok} hisoboti» → «{SmartBlok} ҳисоботи» → qavslar olib tashlanadi →
 * «SmartBlok ҳисоботи». Aks holda «Excel» → «Эхкел» bo'lib chiqadi.
 */
export const cyr = (text: string): string => toCyrillic(text).replace(/\{([^{}]*)\}/g, '$1');
